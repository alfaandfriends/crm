...
<template>
    <BreezeAuthenticatedLayout>
        <h1 class="text-xl font-medium px-1">Edit Pipeline</h1>
...
export default {
    data(){
        return {
            ...
            form: {
                pipeline_id: this.$page.props.pipeline_info.id ?? '',
                assign_to: this.$page.props.pipeline_info.assignee_user_id ?? '',
                assignee_name: this.$page.props.pipeline_info.assignee_name ?? '',
                date_start: this.$page.props.pipeline_info.date_start ?? '',
                lead_source: this.$page.props.pipeline_info.lead_source_id ?? '',
                school_name: this.$page.props.pipeline_info.school_name ?? '',
                school_type: this.$page.props.pipeline_info.school_type_id ?? '',
                branch_number: this.$page.props.pipeline_info.branch_numbers ?? '',
                address: this.$page.props.pipeline_info.school_address ?? '',
                principal_name: this.$page.props.pipeline_info.principal_name ?? '',
                principal_phone_number: this.$page.props.pipeline_info.principal_phone_number ?? '',
                pic_name: this.$page.props.pipeline_info.pic_name ?? '',
    ...
    methods: {
        ...
        submit(){
            this.$inertia.post(route('pipelines.update'), this.form)
        },
        async downloadQuotation(){
            if(!this.form.quotation.name){
                const response = await axios.get(this.form.quotation_path, {
                    responseType: 'blob'
                });
                ...
                this.createBlob(this.form.quotation)
            }
        },
        async downloadContract(){
            if(!this.form.contract.name){
                const response = await axios.get(this.form.contract_path, {
                    responseType: 'blob'
                });
                const blob = new Blob([response.data], { type: response.data.type });
                const url = URL.createObjectURL(blob);
                ...
                this.createBlob(this.form.contract)
            }
        },
        createBlob(file) {
            if (file) {
